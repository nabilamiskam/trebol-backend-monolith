...
class CompleteOrderUseCaseTest {
    ...
    void setUp() {
        ...
        useCase = new CompleteOrderUseCase(transitionService);
    }

    @Test
    void complete_delegatesToTransitionServiceWithCompleteCommand() throws Exception {
        OrderPojo input = new OrderPojo();
        Order expected = Order.builder().id(10L).build();

        when(transitionService.transition(same(input), eq(OrderTransitionCommand.COMPLETE)))
            .thenReturn(expected);

        Order result = useCase.complete(input);

        assertSame(expected, result);
        verify(transitionService).transition(same(input), eq(OrderTransitionCommand.COMPLETE));
    }

    @Test
    void complete_whenInvalidTransition_throwsBadInputException() {
        OrderPojo input = new OrderPojo();
    ...
    @Test
    void complete_whenOrderNotFound_bubblesUpEntityNotFoundException() {
        OrderPojo input = new OrderPojo();
